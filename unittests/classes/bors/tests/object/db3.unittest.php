...
class bors_tests_object_db3_unittest extends PHPUnit_Framework_TestCase
{
    public function testObject()
    {
		$object = object_load('bors_tests_object_db3', -12345);
        $this->assertNull($object);
		$object = object_new_instance('bors_tests_object_db3', array(
			'id' => '123',
			'title' => 'test',
		));
		$object->store();
		...
		$first = objects_first('bors_tests_object_db3', array('title' => 'test'));
		...
		$object = objects_first('bors_tests_object_db3', array('create_time' => $time));
        $this->assertNotNull($object);
