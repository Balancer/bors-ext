...
//$text = preg_replace("/манд(а[^р]|а[^т]|и|е|у|ой|ы)/u","м***",$text);
		...
		$text = preg_replace_callback("/(?<!(ол|ст))([еeё])([б][aаиуy])(ть|л|сь|\b)/ui", 'blib_obscene::stars2', $text);
...
//			$text = preg_replace("/([СсCc])[УуYy][КкKk]([аАaAиИеЕуУyY]|ой)/u","\$1**\$2",$text);
	...
	{
		$allowed = 'ансамбля Джебат дебаты колебания колебать колебаться постебаться '
